  ...
  renderTable({ title, data, weights }) {
    try {
      ...
      for (const [idx, pkgSet] of data.entries()) {
        ...
        table.push([
          chalk`{bold ${pkgSet.source.name || ''}}`,
          chalk`{bold ${pkgSet.source.recommendedVersion || 'No Insight'}}`,
          chalk`{bold ${this.trimStringToLength(pkgSet.source.license, 4) ||
            ''}}`,
          chalk`{bold ${this.trimStringToLength(
            pkgSet.source.description,
            20
          ) || ''}}`,
          chalk`{bold ${this.boolToString(pkgSet.source.deprecated) || ''}}`,
          chalk`{bold ${this.fractionToPercentageString(pkgSet.source.usage) ||
            'No Insight'}}`,
          chalk`{bold ${this.fractionToPercentageString(calculatedScore) ||
            ''}}`
        ])
