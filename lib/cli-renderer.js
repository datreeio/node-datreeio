...
class Renderer {
  ...
  createPackageRow(data, type) {
    let style
    if (type === SOURCE_PACKAGE_TYPE) style = chalk.bold
    else if (type === ALTERNATIVE_PACKAGE_TYPE) style = chalk
    ...
    return [
      ...
      `${style(Number(data.calculatedScore) || '')}`
    ]
  }
  createVersionRow(data) {
    const style = chalk.bold
    return [
      `${style(data.name || '')}`,
      ...
      for (const [idx, pkgSet] of data.entries()) {
        ...
        if ('alternatives' in pkgSet) {
          for (const alternative of pkgSet.alternatives) {
            ...
            table.push(
              this.createPackageRow(alternative, ALTERNATIVE_PACKAGE_TYPE)
            )
          }
        }
      }
