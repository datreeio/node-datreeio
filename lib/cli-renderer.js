...
class Renderer {
  ...
  renderTable ({ title, data, weights }) {
    try {
      ...
      for (const [idx, pkgSet] of data.entries()) {
        ...
        let calculatedScore
        if (Object.keys(weights).length > 0) calculatedScore = score.calculatePackageScore(pkgSet.source, weights, true)
        ...
        table.push([
          ...
          chalk`{cyanBright.bold ${this.fractionToPercentageString(pkgSet.source.usage) || 'No Insight'}}`,
        ...
        if ('alternatives' in pkgSet) {
          for (const alternative of pkgSet.alternatives) {
            let calculatedScore
            if (Object.keys(weights).length > 0) calculatedScore = score.calculatePackageScore(alternative, weights, true)
  ...
  renderFooter (compareList) {
    for (const pkg of compareList) this.queue.push(chalk`{blue.bold https://platform.datree.io/pkg/single-package/${pkg}}`)
  }
}
