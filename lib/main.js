...
async function main () {
  try {
    ...
    renderer.renderLogo()
    if (insightsResponse.insight.length > 0) {
      renderer.renderTable({ title: 'Alternative Packages', data: insightsResponse.insight, weights: insightsResponse.userWeights })
      renderer.renderFooter(insightsResponse.insight.map(pkgSet => pkgSet.source.name))
    }
  } catch (err) {
  }
}
